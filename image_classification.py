...
def preprocess_image(img):
    ...
    for i in range(start_pos, end_pos):
        ...
        while True:
            ...
            if (int(imgn[k][i]) == 255) and (int(imgn[k - 1][i]) != 255):
                break
...
def load_image():
    uploaded_file = st.file_uploader(label='Выберите изображение для распознавания', key="<image>")
...
def model_predict(models, x):
    ...
    for i in range(len(models)):
        s = '**imgn:**' + str(type(np.expand_dims(x, -1))) + '\n' + str(np.expand_dims(x, -1).shape)
        st.write(s)
...
st.title('Распознавание дефектов сварных соединений')
